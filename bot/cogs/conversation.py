"""
Conversation cog — handles all message flow (check-ins, chat, scheduled messages).
Replaces checkin.py.
"""
import os
...
class Conversation(commands.Cog):
    ...
    async def checkin(self, interaction: discord.Interaction):
        ...
        reply = await agent.ask(prompt, topics=["checkin", "manual"])
    ...
    async def send_morning_checkin(self, channel: discord.TextChannel):
        ...
        reply = await agent.ask(prompt, topics=["checkin", "morning", "scheduled"])
    ...
    async def send_eod_review(self, channel: discord.TextChannel):
        ...
        reply = await agent.eod_review(open_list, completed_list)
